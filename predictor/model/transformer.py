...
def get_data(filename):
    ...
    series = pd.read_csv(filename, usecols=['value']) / 100
    series = np.sort(np.lib.stride_tricks.sliding_window_view(series, (10, 1)), axis=2)[:, :, int(10 * 0.75) - 1, :]
    # series = np.max(np.lib.stride_tricks.sliding_window_view(series, (10, 1)), axis=2)
    # scaler = MinMaxScaler(feature_range=(-1, 1))
    # series = scaler.fit_transform(series.values.reshape(-1, 1)).reshape(-1)
    series = series.reshape(-1)
    train_samples = int(0.75 * len(series))
